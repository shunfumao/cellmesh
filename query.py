...
def prob_test(
  ...
  for cell_id, cell_name in all_cells:
    ...
    genes_pmids_count = set(
      get_cell_genes_pmids(
        cell_id, db_dir=db_dir, species=species, threshold=params["db_cnt_thre"], uppercase_gene_names=True))
...
def test_prob_test():
  ...
  #based on CellMeSH, the top retrieval should be:
  ...
  cell_prob_vals = prob_test(genes=genes, params=params, species='mouse')
